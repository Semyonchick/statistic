<template>
    <div>
        ...
        <table v-if="date" width="100%" cellspacing="0" cellpadding="0">
            ...
            <tbody>
            <tr v-for="row in leadStatistic" class="dataField">
                <th>{{row.name}}</th>
                <td>{{row.price}}</td>
                <td>{{row.priceDeal}}</td>
                <td class="primary">{{row.leads || '-'}} <span v-if="row.leadsPercent">/ {{row.leadsPercent}}%</span>
                </td>
                <td class="success">{{row.leadDeals || '-'}}<span v-if="row.leadDealsPercent">/ {{row.leadDealsPercent}}%</span>
                </td>
                <td class="bad">{{row.leadCanceled || '-'}}<span
                        v-if="row.leadCanceledPercent">/ {{row.leadCanceledPercent}}%</span></td>
            </tr>
            </tbody>
            <tfoot>
            <tr class="dataField">
